...
+
WARNING: Make sure you use Clojure `1.10.0`/ClojureScript `1.10.520` or higher; if you're using figwheel-main, use `0.2.1-SNAPSHOT` or higher; don't override Ghostwheel's dependencies with lower versions or you might get link:https://github.com/gnl/ghostwheel/issues/9[strange behaviour].
+
[TIP]
--
...
[NOTE]
--
You can disable Ghostwheel manually:
- Clojure – launch the REPL / build with the JVM system property `-Dghostwheel.enabled=false`
- ClojureScript – either set `-Dghostwheel.enabled=false` or add `:external-config {:ghostwheel {:enabled false}}` to the compiler options in your build config.
That being said, it's recommended to use the stubs artifact in production as it ensures that no tracing or testing code can be generated and it's significantly more reliable in reducing dependencies and build size than relying on dead code elimination alone.
--
+
[NOTE]
--
...
When `gnl/ghostwheel.stubs` is used, any configuration is ignored and Ghostwheel simply cannot be enabled.
--
...
|6
|Named anonymous functions are expanded by default
|Possibly very noisy debugging
|===
...
[TIP]
--
Add the following devtools config to your compiler options to improve data structure readability when tracing:
```clojure
:external-config
{:devtools/config {:max-print-level 4
...
[[adhoc-tracing]]
TIP: Use `|>` or `tr` (they are aliases) to wrap any expression for more granular tracing or if you are only interested in Ghostwheel's tracing functionality. They work standalone, default to trace level `6` and will generally do the right thing depending on the type of wrapped form – functions and threading macros get proper in-depth tracing, while other expressions are pretty-printed and returned.
